 ...
 * Other than ensuring message order is correct, all message creation and
 * parsing is handled through the current command handed to sendMessage.
 * 
...
public class IRodsConnection {
	...
	private static final Logger LOG = LoggerFactory.getLogger(IRodsConnection.class);
	...
	/**
	 * Return true if an response inputstream is still available, available
	 * means the last call has returned an input stream that has not been closed
	 * yet. If the last returned no input stream, then this returns true
	 * 
	...
	/**
	 * Send a message over this connection with only a header that has a type
	 * and intInfo set. This is useful for authentication requests
	 * 
	 * @param msgType
	 *            type of message
	 * @param intInfo
	 *            value to set for int info
	 * @return server response
	 * @throws java.io.IOException
	 */
	public ResultMessage sendMessage(HeaderTypeEnum msgType,
			ApiNumberEnum intInfo) throws IOException {
	 ...
	 *            header message type
	 * @param bodyPart
	 *            body to send, must not be null
	 * @return server response
	 * @throws java.io.IOException
	 */
	public ResultMessage sendMessage(HeaderTypeEnum msgType,
			ApiNumberEnum intInfo, IRodsPI bodyPart) throws IOException {
		return sendMessage(msgType, intInfo, bodyPart, null, null);
	}
	 ...
	 *            length of bitstream
	 * @param bitstreamData
	 *            callback to write data to stream. Must not write more than
	 *            bitstreamLength bytes
	 * @return server response
	 * @throws java.io.IOException
	 */
	public ResultMessage sendMessage(HeaderTypeEnum msgType,
		...
		return sendMessage(msgType, intInfo, bodyPart, null, bitstreamData);
	}

	/**
	 * mostly the same as send message except that resultmessage is returned out
	 * of band. This allows the bitstream callback to hold the outputstream open
	 * after it returne from writebits
	 * 
	 */
	public synchronized void nonblockingSendMessage(HeaderTypeEnum msgType,
		...
		if (bitstreamData == null) {
			...
			try {
				...
				MsgHeaderRequest_PI msgHeader = createHeader(msgType, intInfo,
						bodyPart, null, bsLen);
				...
				try {
					...
					transmitMessage(msgHeader, bodyPart, null, bitstreamData,
							callback);
					// LOG.info("Transmit time: " + (System.currentTimeMillis()
					// - start));
			...
			// }
				...
				closeConnection();
				callback.exceptionReceived(new IOException("Uncaught exception"
						+ e.getMessage()));
	...
	/**
	 * Read the response from the irods from the wire. This just tosses
	 * ioexceptions up the chain, no closing is done.
	 * 
	 * @return
	 * @throws java.io.IOException
	 *             if network read failed
	 */
	private ResultMessage handleResponse() throws IOException {
		...
		int responseHeaderSize = RodsUtil
				.parseInt(RodsUtil.readBytes(4, input));
		if (debug) {
			LOG.debug("Read response header size: " + responseHeaderSize);
		}
		if (responseHeaderSize < 0 || responseHeaderSize > MAX_HEADER_SIZE) {
			LOG.info("Unexpected header size " + responseHeaderSize
					+ " is this even irods?");
			throw new IOException("Unexpected header size "
					+ responseHeaderSize);
		}
		...
		if (responseHeaderPI.getBsLen() > 0) {
			activeInput = new RodsInputStream(input,
					responseHeaderPI.getBsLen());
		} else {
			activeInput = null;
		}
	...
	private void transmitMessage(MsgHeaderRequest_PI msgHeaderPI,
			IRodsPI bodyPart, RError_PI errorPart,
			BitstreamCallback bitstreamData, ResultCallback callback)
			throws IOException {
		byte[] headerSize;
		byte[] header;
		...
		if (isClosed()) {
			throw new IOException(
					"Attempt to send command to closed connection");
		}
		...
		if (msgHeaderPI.getBsLen() > 0) {
			...
			if (debug) {
				LOG.debug("Sending bitstream, size: " + msgHeaderPI.getBsLen());
			}
	...
	public synchronized void closeConnection() {
		...
		try {
			MsgHeaderRequest_PI header = createHeader(
					HeaderTypeEnum.RODS_DISCONNECT,
